...
# List of models to try in order of preference (updated to latest Gemini models)
MODEL_PRIORITY = [
    "gemini-2.5-flash",  # Most balanced model, widely available
    ...
    # Fallback to older models if new ones aren't available
    "gemini-1.5-flash",
...
def get_llm(model_name=None):
    ...
    for model in models_to_try:
        try:
            # Try with standard model name first (works with API key)
            ...
            print(f"✓ Initialized Gemini model: {model}")
...
async def analyze_research(state: ResearchState) -> ResearchState:
    """Node: Analyze raw research data and generate structured summary"""
    if not ensure_llm_initialized():
        return {**state, "research_summary": {"error": "LLM not initialized. Please check GEMINI_API_KEY and model configuration."}}
    ...
    try:
        ...
        try:
            ...
            return {**state, "research_summary": parsed}
        except json.JSONDecodeError as e:
            # If LLM returned plain text, wrap it
            print(f"JSON parsing error: {e}")
            return {**state, "research_summary": {"raw_text": content, "error": "Failed to parse as JSON"}}
    except Exception as e:
        # Handle API errors
        error_msg = f"Error calling Gemini API: {str(e)}"
        print(error_msg)
        return {**state, "research_summary": {"error": error_msg, "raw_data": raw_data}}

# Build LangGraph workflow
def create_research_graph():
    """Create the LangGraph workflow for research analysis"""
...
async def summarize_research_with_numbers(company: str, raw_data: dict):
    """Summarize research data using LangGraph workflow"""
    initial_state = {
        "company": company,
        "raw_data": raw_data,
        "research_summary": {},
